...
function doTitleSearch(args) {
    ...
    return new Promise(function (resolve, reject) {
        connection.query("SELECT * FROM item, author WHERE item.i_a_id = author.a_id AND substring(soundex(item.i_title),0,4)=substring(soundex('" + args.search_key + "'),0,4) ORDER BY item.i_title limit 50", function (err, rows, fields) {
            if (err) {
                reject(err);
            }
            else {
...
module.exports = async (event, context) => {
    return await doTitleSearch(event.body);
}
