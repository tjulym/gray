...
function best_sellers(args) {
    ...
    html += `<!DOCTYPE HTML>
    <HTML><HEAD><TITLE> Best Sellers: ${subject}</TITLE></HEAD>
    <BODY BGCOLOR="#ffffff">
    ...
    <H2 ALIGN="center">Best Sellers Page - Subject: ${subject}</H2>
    `;
    var params = {
        C_ID: C_ID,
    ...
    return openfaas.invoke('promotionalprocessing', params, {isJson: true, isBinaryResponse: false}).then(result => {
        ...
        return openfaas.invoke('getbestsellers', params, {isJson: true, isBinaryResponse: false}).then(result => {
            res = result.body;
            var books = res.books;
            var i;
...
module.exports = async (event, context) => {
    return await best_sellers(event.query);
}
