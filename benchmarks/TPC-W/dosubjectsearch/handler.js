...
function doSubjectSearch(args) {
    ...
    return new Promise(function (resolve, reject) {
        connection.query("SELECT * FROM item, author WHERE item.i_a_id = author.a_id AND item.i_subject = '" + args.search_key + "' ORDER BY item.i_title limit 50", function (err, rows, fields) {
            if (err) {
                reject(err);
            }
            else {
...
module.exports = async (event, context) => {
    return await doSubjectSearch(event.body);
}
