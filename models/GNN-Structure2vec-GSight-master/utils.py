        ...
        with open(f_name) as inf:
            for line in inf:
                ...
                if (g_info['fname'] not in name_dict):
                    ...
                    name_num += 1
                    print("%s, %d",g_info['fname'],name_num)
    return name_dict
...
def train_epoch(model, s_train_list, s_feature_list, s_label_list):

    cum_loss = 0.0
    #print(len(s_train_list[0]))
    for index in range(len(s_train_list)):
        X1 = s_train_list[index]
        mask1 = s_feature_list[index]
        y = s_label_list[index]
        loss = model.train([X1], [mask1], [y])
        cum_loss += loss
        #print(loss)

    return cum_loss / len(s_train_list)
...
def get_auc_epoch(model, sub_train_test_list, sub_feature_test_list, sub_label_test_list):
    tot_diff = []
    tot_truth = []

    cum_loss = 0
    for index in range(len(sub_train_test_list)):
        X1 = sub_train_test_list[index]
        mask1 = sub_feature_test_list[index]
        y = sub_label_test_list[index]

        diff = model.calc_diff([X1], [mask1])
    #    print diff
        loss = abs(diff + y) / y
        cum_loss += loss

    return cum_loss/len(sub_train_test_list)
