...
# X: featuresMatrix 10x16  msg_mask 10x10  N_x 16 N_embed 64
...
class graphnn(object):
    def __init__(self,
        ...
        with tf.device(device):
            ...
            # for lc-lc lc-be
            cos = tf.reduce_sum(embed1, 1) / tf.sqrt(tf.reduce_sum(embed1**2, 1) + 1e-10)
            # for be-be
            #cos = tf.reduce_sum(embed1, 1)

            diff = -cos
            self.diff = diff
            loss = tf.reduce_mean( abs(diff + label)/label )
            self.loss = loss
    ...
    def calc_diff(self, X1, mask1):
        diff, = self.sess.run(fetches=[self.diff], feed_dict={self.X1:X1, self.msg1_mask:mask1})
        return diff
    ...
    #         self.X2:X2,self.msg1_mask:mask1,self.msg2_mask:mask2,self.label:y})
    #     return loss
    def train(self, X1, mask1, y):
        loss,_ = self.sess.run([self.loss,self.optimizer],feed_dict={self.X1:X1,self.msg1_mask:mask1,self.label:y})
        return loss
