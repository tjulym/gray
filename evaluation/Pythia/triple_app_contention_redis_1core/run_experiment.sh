...
#
# INPUT: suite bmark cores suite2 bmark2 cores2 suite3 bmark3 cores3 rep
#
...
EXPERIMENT_LOG3="logs/${EXPERIMENT_NAME}.log3"
...
# Run the batch application 3
if [ "x${SUITE3}" == "xparsec" ]; then
	run_parsec "${BMARK3}" "${BUBBLE_CORE3}" >> "${EXPERIMENT_LOG3}" &
	if [ $? -ne 0 ]; then
        echo "Error: Failed to run benchmark"
        exit 2
	fi
elif [ "x${SUITE3}" == "xspec_int" -o "x${SUITE3}" == "xspec_fp" ]; then
	run_spec "${BMARK3}" "${BUBBLE_CORE3}" >> "${EXPERIMENT_LOG3}" &
	if [ $? -ne 0 ]; then
        echo "Error: Failed to run benchmark"
        exit 2
	fi
