...
addpath('../MatlabCommon/','../../packages/glmnet_matlab','../MatlabCommon/export_fig-master/');
% Options
filename = Create_BM();
m   = length(filename);
k   = 4;
...
fit = options_genericLasso(fitt_q{i});
lassoPlot(fitt_q{i}.beta,fit,'PlotType','Lambda','XScale','log');
lassoPlot(fitt_q{i}.beta,fit,'PlotType','CV','XScale','log')
...
for j = 1:N    
    ...
    for i = 1:(k-1)
        for l = 1:length(pp)
            [Yhat_l{i,j,l},acc_est_l(i,j,l),acc_est_train_l(i,j,l) ,~ ,beta_l{i,j,l},fitt_l{i,j,l}] =...
                glmnet_new( Xsummary{i}, Y{i}, pp(l),'model','linear','boxcox',0,'var_sel',0,'alpha',0);
            fprintf('p = %5.2f, multi-%d, acc_est = %5.2f, acc_train = %5.2f \n',...
                    pp(l),i+1,acc_est_l(i,j,l),acc_est_train_l(i,j,l));
        end
    end
