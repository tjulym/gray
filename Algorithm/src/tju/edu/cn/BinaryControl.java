...
public class BinaryControl {
	public static void main(String[] args) {
		...
		int min_Num = 0, max_Num = orderAPP.length, mid_Num = 0;
		boolean SLA_violation = false;
//		SLA_violation = scheduler(orderServer, orderAPP, map, max_Num);
		if (!SLA_violation) {
//			System.out.println(max_Num);
			while (min_Num < max_Num) {
				mid_Num = (min_Num + max_Num) / 2;
				SLA_violation = scheduler(orderServer, orderAPP, map, mid_Num);
				if (SLA_violation) {
		...
		// scheduler(orderServer, orderAPP, map, max_Num);
	 ...
	 * @return
	 */
	public static boolean compareSLA(String[] orderServer, float targetSLA) {
	 ...
	 * @return
	 */
	public static boolean scheduler(String[] orderServer, String[] orderAPP, Map<String, String> map, int number) {
		for (int i = orderAPP.length - 1; i >= (orderAPP.length - number); i--) {
			updateBubbleSort(orderServer, Double.parseDouble(orderAPP[i]), map);
		}
		getSchedulerResult(orderServer, map);
		// compareSLA(orderServer, targetSLA)
		return true;
	}
